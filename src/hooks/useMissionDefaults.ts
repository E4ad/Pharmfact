...
export function buildMissionDefaults(state: AppState, activePharmacienId?: string) {
  ...
  const pharmacyDaySchedule = getPharmacyScheduleForDate(defaultPharmacie?.weeklySchedule, todayIso());
  ...
  return {
    ...
    scheduleDefaults: {
      startTime: pharmacyDaySchedule?.enabled ? pharmacyDaySchedule.startTime ?? pharmacien?.defaultStartTime ?? options.missionDefaults.defaultStartTime : pharmacien?.defaultStartTime ?? options.missionDefaults.defaultStartTime,
      endTime: pharmacyDaySchedule?.enabled ? pharmacyDaySchedule.endTime ?? pharmacien?.defaultEndTime ?? options.missionDefaults.defaultEndTime : pharmacien?.defaultEndTime ?? options.missionDefaults.defaultEndTime,
      breakMinutes: pharmacien?.defaultBreakMinutes ?? options.missionDefaults.defaultBreakMinutes,
    },
    mealDefaults: {
...
export function resolveBreakMinutes(params: { pharmacie?: Pharmacie; pharmacien?: Pharmacien; globalDefault: number }) {
  return params.pharmacien?.defaultBreakMinutes ?? params.globalDefault;
}
